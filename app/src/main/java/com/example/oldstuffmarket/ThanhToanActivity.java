                                        ...
                                        databaseReference.child("SanPham").addChildEventListener(new ChildEventListener() {
                                            ...
                                            public void onChildAdded(@NonNull DataSnapshot snapshot, @Nullable String previousChildName) {
                                                if(snapshot.getValue(SanPham.class).getsID().equals(sanPhamID)){
                                                    ...
                                                    Date date = new Date();
                                                    OrderData orderData = new OrderData(donHangID, userID, nguoiBanID, dateFormat.format(date), edtLienHe.getText().toString(), edtDiaChi.getText().toString(), sanPhamOrder, 2, 0, tongGiaTriDonHang);
                                            ...
                                            databaseReference.child("SanPham").addChildEventListener(new ChildEventListener() {
                                                ...
                                                public void onChildAdded(@NonNull DataSnapshot snapshot, @Nullable String previousChildName) {
                                                    if(snapshot.getValue(SanPham.class).getsID().equals(sanPhamID)){
                                                        ...
                                                        DateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
                                                        Date date = new Date();
                                                        OrderData orderData = new OrderData(donHangID, userID, nguoiBanID, dateFormat.format(date), edtLienHe.getText().toString(), edtDiaChi.getText().toString(), sanPhamOrder, 3, 0, tongGiaTriDonHang);
                                                        databaseReference.child("DonHang").child(donHangID).setValue(orderData);
                                                        finish();
                                                        intent = new Intent(v.getContext(), UserMainActivity.class);
                                                        intent.putExtra("UserName", userName);
